    ...
    end;
{
    ...
    map((.[].ABpath[:-23]) as $ABpath | .[].data[] | select(.ZUNIQUEID | endswith("ABPerson")) |
        ...
        (.ZEMAILADDRESS | fromjson | map(select(. != null and . != "")) | unique | sort | join(" ")) as $EMAILS |
        ([.ZJOBTITLE, .ZORGANIZATION, $PHONES, $EMAILS, .ZSTREET, .ZTEXT] | map(select(. != null and . != "")) | length) as $richness |
        ...
        (if (.ZFIRSTNAME == null and .ZLASTNAME == null and .ZSUFFIX == null and .ZORGANIZATION == null) then "No Name" elif (.ZFIRSTNAME == null and .ZLASTNAME == null and .ZSUFFIX != null) then .ZSUFFIX elif $isNotORG then ([.ZFIRSTNAME, .ZLASTNAME] | map(select(.)) | join(" ")) else .ZORGANIZATION end) as $title |
    	{
            ...
            "subtitle": (
                (if $isNotORG then ([.ZJOBTITLE, .ZORGANIZATION] | map(select(.)) | join(" • ")) else "" end)
                +
                (if ($ABpath | contains("DDFC607B-A170-4FAA-B67A-0768462EA8A7")) then "" else " ↗ Contacts App" end)
            ),
            "arg": .ZUNIQUEID,
            "autocomplete": $title,
            "valid": true,
            "dedupe_key": ("person:" + ($title | ascii_downcase | gsub("\\s+"; " ")) + "|" + ((.ZORGANIZATION // "") | ascii_downcase | gsub("\\s+"; " ")) + "|" + ((.ZJOBTITLE // "") | ascii_downcase | gsub("\\s+"; " ")) + "|" + (if (($EMAILS | length) > 0) then ($EMAILS | ascii_downcase) elif (($PHONES | gsub("[^0-9]"; "") | length) > 0) then ($PHONES | gsub("[^0-9]"; "")) else "" end)),
    ...
    ) | sort_by(.dedupe_key, - .richness, .sortindex) | unique_by(.dedupe_key) | sort_by(.sortindex) | map(del(.dedupe_key, .sortindex, .richness)) else
